...
const testimonials = [
  {quote:"She spent two hours on the consultation alone. The result felt like she'd read my mind — and then improved it.",name:"Emilia R.",city:"Wrocław"},
  ...
  {quote:"The piece still stops people cold. It looks impossible. That's exactly what I wanted.",name:"Petra L.",city:"Warsaw"},
];

function useCounter(t,dur=1800,active=false){
  const [v,setV]=useState(0);
  useEffect(()=>{if(!active)return;let s=null;const step=ts=>{if(!s)s=ts;const p=Math.min((ts-s)/dur,1);setV(Math.floor(p*t));if(p<1)requestAnimationFrame(step);};requestAnimationFrame(step);},[active,t,dur]);
  return v;
}
function useVisible(ref){
  const [vis,setVis]=useState(false);
  useEffect(()=>{if(!ref.current)return;const o=new IntersectionObserver(([e])=>{if(e.isIntersecting)setVis(true);},{threshold:.15});o.observe(ref.current);return()=>o.disconnect();},[ref]);
  return vis;
}
  ...
  return(
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,zIndex:600,background:"rgba(0,0,0,.95)",display:"flex",alignItems:"center",justifyContent:"center",animation:"fadeIn .35s ease"}}>
      <div style={{position:"relative",width:"min(900px,90vw)",aspectRatio:"16/9"}}>
        ...
        <p style={{...mono,position:"absolute",bottom:-34,left:0,fontSize:9,letterSpacing:".15em",color:"#333"}}>{label}</p>
      </div>
    </div>
  );
}
...
function EventCard({ev, cardIndex, onVideoOpen}){
  ...
  const goTo = useCallback((idx)=>{
    ...
    setFading(true);
    setTimeout(()=>{setCur(idx);setFading(false);},320);
  },[cur,fading]);
  ...
  return(
    ...
    >
      ...
      <div style={{position:"absolute",inset:0,opacity:fading?0:1,transition:"opacity .32s ease"}}>
        {isVideo && hasRealSrc ? (
          ...
          <div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",opacity:.18,transition:"opacity .5s",...( hov?{opacity:.28}:{})}}>
          ...
          /* placeholder */
          <div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",opacity:hov?.22:.14,transition:"opacity .5s"}}>
              {arts[cardIndex%arts.length]}
            </div>
          </div>
        )}
      ...
      {/* gradient shade */}
      <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,.9) 0%,rgba(0,0,0,.08) 48%,transparent 100%)",pointerEvents:"none",zIndex:2}}/>
      ...
      {isVideo && (
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",zIndex:4,pointerEvents:"none"}}>
          <div style={{position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div style={{position:"absolute",width:72,height:72,borderRadius:"50%",border:"1px solid rgba(255,255,255,.07)",animation:"ring 2.2s ease-in-out infinite"}}/>
            <div style={{width:50,height:50,borderRadius:"50%",border:"1px solid",borderColor:hov?"rgba(255,255,255,.65)":"rgba(255,255,255,.22)",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .3s",background:hov?"rgba(255,255,255,.07)":"transparent"}}>
              <div style={{width:0,height:0,borderStyle:"solid",borderWidth:"7px 0 7px 12px",borderColor:"transparent transparent transparent rgba(255,255,255,.85)",marginLeft:3}}/>
            </div>
          </div>
        </div>
      )}
      ...
      {/* media type badge */}
      <div style={{position:"absolute",top:14,right:14,zIndex:6,...mono,fontSize:7,letterSpacing:".2em",textTransform:"uppercase",padding:"3px 9px",background:"rgba(8,8,8,.88)",border:"1px solid",borderColor:hov?"#333":"rgba(255,255,255,.1)",color:hov?"#888":"#555",transition:"all .3s"}}>
        {isVideo?"▶ Video":"Photo"}
      </div>

      {/* counter */}
      {count > 1 && (
        <div style={{position:"absolute",top:14,left:14,zIndex:6,...mono,fontSize:8,letterSpacing:".18em",color:hov?"rgba(255,255,255,.65)":"rgba(255,255,255,.3)",transition:"color .3s"}}>
      ...
      {/* dot navigation */}
      {count > 1 && (
        <div style={{position:"absolute",bottom:106,left:0,right:0,display:"flex",justifyContent:"center",gap:6,zIndex:6}}>
          {media.map((_,i)=>(
            <div key={i} onClick={e=>{e.stopPropagation();goTo(i);}}
              style={{width:i===cur?16:4,height:4,borderRadius:2,background:i===cur?"rgba(255,255,255,.8)":"rgba(255,255,255,.22)",transition:"all .3s",cursor:"pointer"}}/>
          ))}
        </div>
      )}

      {/* info block */}
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"18px 20px 20px",zIndex:5,transform:hov?"translateY(0)":"translateY(5px)",transition:"transform .4s cubic-bezier(.25,.46,.45,.94)"}}>
        <div style={{display:"flex",gap:5,marginBottom:8,flexWrap:"wrap"}}>
          {[ev.year,ev.location,ev.kind].map(t=>(
            <span key={t} style={{...mono,fontSize:7,letterSpacing:".18em",textTransform:"uppercase",color:"#777",border:"1px solid rgba(255,255,255,.1)",padding:"2px 7px"}}>{t}</span>
          ))}
        </div>
        <p style={{fontSize:"clamp(17px,2.2vw,22px)",fontWeight:300,lineHeight:1.15,marginBottom:5,position:"relative",display:"inline-block"}}>
          {ev.event}
          <span style={{position:"absolute",bottom:-3,left:0,height:1,background:W,transition:"width .5s cubic-bezier(.25,.46,.45,.94)",width:hov?"100%":"0"}}/>
        </p>
        <p style={{...mono,fontSize:10,lineHeight:1.7,color:"#555",overflow:"hidden",maxHeight:hov?56:0,transition:"max-height .4s .05s,opacity .4s",opacity:hov?1:0}}>{ev.caption}</p>
      </div>
    </div>
  );
}
    ...
    <div style={{background:BG,color:W,...serif,minHeight:"100vh",overflowX:"hidden"}}>
      ...
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:200,padding:"0 24px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(14,14,14,.97)":"transparent",borderBottom:scrolled?`1px solid ${BD}`:"none",backdropFilter:scrolled?"blur(10px)":"none",transition:"background .4s,border .4s"}}>
        ...
        <div className="nav-links" style={{display:"flex",gap:28,alignItems:"center"}}>
          ...
          <button onClick={()=>setIgOpen(true)} style={{background:"none",border:"1px solid #2a2a2a",borderRadius:"50%",width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"border-color .3s",marginLeft:4}} onMouseEnter={e=>e.currentTarget.style.borderColor="#fff"} onMouseLeave={e=>e.currentTarget.style.borderColor="#2a2a2a"}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#aaa" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4.5"/><circle cx="17.5" cy="6.5" r="1" fill="#aaa" stroke="none"/>
            </svg>
          </button>
        </div>
        <button className="hamburger" onClick={()=>setMenuOpen(true)} style={{display:"none",flexDirection:"column",gap:5,background:"none",border:"none",cursor:"pointer",padding:4}}>
          <span style={{display:"block",width:24,height:1.5,background:W}}/><span style={{display:"block",width:24,height:1.5,background:W}}/><span style={{display:"block",width:18,height:1.5,background:W}}/>
        </button>
      </nav>
      ...
      <section id="events" style={{background:BG3,borderTop:`1px solid ${BD}`}}>
        ...
        <div className="mosaic">
          {events.map((ev,i)=>(
            <EventCard key={i} ev={ev} cardIndex={i} onVideoOpen={(src,label)=>setVideo({src,label})}/>
          ))}
        </div>
      ...
      <div className={`ig-ov${igOpen?" open":""}`} onClick={e=>{if(e.target===e.currentTarget)setIgOpen(false);}}>
        ...
        <div style={{textAlign:"center",padding:"0 32px"}}>
          ...
          <p className="ig-desc">Work in progress, healed pieces, convention updates, and guest spot announcements.</p>
